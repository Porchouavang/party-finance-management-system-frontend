...
function IncomeComponent() {
    ...
    const fetchIncome = async () => {
        ...
        try {
            const response = await fetch(
                `http://192.168.100.134:3001/api/finance/select-details-income-by-party/${id}`,
                {
                    headers: {
  ...
  const exportToExcel = () => {
    ...
    saveAs(data, `income_report_${id}.xlsx`);
  };
  const handlePrint = (id) => {
  const oldTitle = document.title;
  document.title = `ລາຍຮັບ${id}`;
  window.print();
                        ...
                        <div className="content">
                            ...
                            <div className="row">
                                <div className="d-flex justify-content-around">
                                    ...
                                    <h2 className="text-center text-bold text-success">ໃບບິນລາຍຮັບ{Income[0]?.party}</h2>
                                    <DateTimeComponent />
                                </div>
                                                ...
                                                <tbody className="fw-bold">
                                                    {Income.map((income, index) => (
                                                        <tr key={income.id || index}>
                                                            ...
                                                            <td className="text-success">{new Date(income.created_at).toLocaleString()}</td>
                                                        </tr>
                                                    ))}
                                                    ...
                                                    <tr className="fw-bold">
                                                        ...
                                                        <td>ລາຍຮັບທັງໝົດ:</td>
                                                        <td className="text-success">
                                                            {Number(SumIncome[0]?.sum_income || 0).toLocaleString()}
                                                        </td>
                                                    </tr>
                                                </tbody>
                                            </table>
